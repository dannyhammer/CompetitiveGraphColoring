...
GRAPH_TYPES = ["Custom","Path", "Cycle", "Wheel", "Complete", "Prebuilt"]
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for vtx in VERTICES:
                    if (is_clicked(vtx["x"], vtx["y"], mouse_x, mouse_y, RADIUS)):
                        ...
                        #if game_state is -1 or game_state is 1:
                        #    game_over = True
...
def run_setup(message, menu_num):
    ...
    while setup_running:
        ...
        for event in pygame.event.get():
            ...
            if menu_num is 1:
                display_list("Graph types:", CRAYONBOX["BLACK"], 35, GRAPH_TYPES)
            # List all available prebuilt graphs for the fourth menu
            elif menu_num is 5:
                display_list("Files to choose from:", CRAYONBOX["BLACK"], 35, PREBUILT_GRAPHS)
...
def display_turn(turn):
    ...
    display = font.render("Player " + str(player) + "'s turn", 1, CRAYONBOX["BLACK"], BACKGROUND)
    
    screen.blit(display, (int((WINDOW_WIDTH / 2) - (display.get_size()[0] / 2)), 10))
...
def display_list(message, color, font_size, items):
    ...
    for i in range(0, len(items)):
        ...
        display = font.render(str(i) + " - " + str(items[i]), 1, color, BACKGROUND)
        screen.blit(display, (x, y))
...
def create_wheel(number_of_vertices, radius, thickness):
    """
    Creates a standard wheel of n vertices on screen

    Follows the same procedure as cycle generation with the
    exception that the first vertex is placed in the center
    and assigned adjacency with the remaining vertices.
    ...
    for i in range(0, number_of_vertices):
        if i is 0:
            vtx_x = int(WINDOW_WIDTH / 2)
            vtx_y = int(WINDOW_HEIGHT / 2)
        else:
            vtx_x = int((WINDOW_WIDTH / 2) + math.cos((i * math.pi * 2) / (number_of_vertices - 1)) * dist_apart)
            vtx_y = int((WINDOW_HEIGHT / 2) + math.sin((i * math.pi * 2) / (number_of_vertices - 1)) * dist_apart)
    ...
    for i in range(0, number_of_vertices):
        ...
        if i is not number_of_vertices - 1 and i is not 0:
            VERTICES[i]["adjacent"].append(VERTICES[i + 1]["ID"])
            VERTICES[i + 1]["adjacent"].append(VERTICES[i]["ID"])
        else:
            VERTICES[i]["adjacent"].append(VERTICES[1]["ID"])
            VERTICES[1]["adjacent"].append(VERTICES[i]["ID"])
...
def create_custom_graph(radius, thickness):
    ...
    # This is for creating new graphs
...
# NOTE: This function is not used in gameplay, but used for data collection
...
def create_graph(radius, thickness):
    ...
    if graph_type > 5 or graph_type < 0:
        graph_type = 1

    # If the graph is not custom or prebuilt, ask for a number of vertices
    if graph_type is not 0 and graph_type is not 5:
    ...
    elif graph_type is 1:
        create_path(num_vertices, radius, thickness)
    elif graph_type is 2:
        create_cycle(num_vertices, radius, thickness)
    elif graph_type is 3:
        create_wheel(num_vertices, radius, thickness)
    elif graph_type is 4:
        create_complete(num_vertices, radius, thickness)
    elif graph_type is 5:
        # Prompt the user to choose from a list of existing graphs
        graph_choice, game_over = run_setup("Choose a graph file", 5)
...
def draw_graph(vertices, radius, thickness):
    ...
    # Draws shadows
    for vtx in vertices:
        pygame.draw.circle(screen, CRAYONBOX["DARK GRAY"], (vtx["x"] + 4, vtx["y"] + 4), radius)

    # Draws the edges
    for vtx in vertices:
        for adj in vtx["adjacent"]:
            draw_line(vtx["x"], vtx["y"], vertices[adj]["x"], vertices[adj]["y"], thickness)
...
def draw_line(x_one, y_one, x_two, y_two, thickness):
    ...
    pygame.draw.line(screen, CRAYONBOX["DARK GRAY"], (x_one + 3, y_one + 3), (x_two + 3, y_two + 3), thickness)
    pygame.draw.line(screen, CRAYONBOX["BLACK"], (x_one, y_one), (x_two, y_two), thickness)
...
def recolor(radius, thickness, colors, keys, vtx_x, vtx_y):
    ...
    If the new color is not the old color and is a legal coloring, recolor
    ...
    try:
        ...
        print("ERROR: You must select a color using 1-" + str(len(colors)) + " first!")
        return
