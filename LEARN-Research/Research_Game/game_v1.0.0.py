...
def centered_message(message, color, size):
    """
...
def display_list(message, color, size, items):
    """
...
def create_graph(radius, thickness):
    ...
    if graph_type is not 0 and graph_type is not 4:
...
def draw_graph(vertices, radius, thickness):
    ...
    # Draws the vertices
    for vtx in vertices:
        draw_circle("WHITE", vtx["x"], vtx["y"], radius, thickness)




def draw_circle(color, x, y, radius, thickness):
    """
    Draws a circle on the screen
    
    Draws an outer black edge and an inner colored circle.
...
def recolor(radius, thickness, colors, keys, vtx_x, vtx_y):
    ...
    if CRAYONBOX[new_color] != current_color and is_legal(vertex, new_color):
        draw_circle(new_color, vtx_x, vtx_y, radius, thickness)
